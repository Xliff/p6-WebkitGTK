...
sub jsc_value_new_number (JSCContext $context, gdouble $number)
  returns JSCValue
  is native(jsc)
  is export
  { * }
...
sub jsc_value_object_define_property_accessor (
  JSCValue $value,
  Str $property_name,
  guint $flags,                   # JSCValuePropertyFlags $flags,
  GType $property_type,
  GCallback $getter,
  GCallback $setter,
  gpointer $user_data,
...
sub jsc_value_new_array_from_strv (JSCContext $context, CArray[Str] $str)
  returns JSCValue
  is native(jsc)
  is export
  { * }
